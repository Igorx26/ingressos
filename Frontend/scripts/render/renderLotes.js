...
import { fetchEventos } from "../fetch/fetchEventos.js";
import { fetchTiposIngressos } from "../fetch/fetchTiposIngressos.js";
import { fetchLoteById } from "../fetch/fetchLoteById.js";
import { fetchLotes } from "../fetch/fetchLotes.js";
...
function renderLotesIndex(lotes) {
  ...
  if (lotes.length === 0) {
    gridLotes.innerHTML = `<p style="margin-top: 50px; font-size:25px; color:red;">Nenhum lote cadastrado.</p>`;
    return;
  } else {
    lotes.forEach((lote) => {
...
async function renderLotesAdmin(lotes) {
  ...
  try {
    ...
    tiposIngressos = await fetchTiposIngressos();
  } catch (error) {
    alert(error.message);
  }
...
async function carregarTiposIngressos() {
  try {
    ...
    // Busca os tipos de ingressos da API
    const tiposIngressos = await fetchTiposIngressos();
...
async function deletarLote(id, nome) {
  ...
  try {
    ...
    renderLotes(); // Recarrega a lista após a exclusão
  } catch (error) {
    alert(error.message);
  }
}
